	...
	for (int j = 0; j < gridSize; j++) {
		...
		for (int i = 0; i < gridSize; i++) {
			...
			// different spacing depending on number of digits: (10 chars with number in center (or slightly to the right)
				...
				for (int spaces = 0; spaces < ((10 - digitsInCell) / 2.0); spaces++) { // left spaces (if 2 digit, 4 left spaces; if 1 digit 5 left spaces)
					currentRow += " ";
				}
				currentRow += currentCell;
				for (int spaces = 0; spaces < ((10 - digitsInCell) / 2); spaces++) { // right spaces (if 2 digit, 4 right spaces; if 1 digit 4 right spaces)
					currentRow += " ";
				}
			}
